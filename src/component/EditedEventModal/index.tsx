...
export const EditedEventModal = () => {
  const { eventSelected, setShowEditedEventModal, daySelected } =
    useCalendarContext()

  const { isLogin, shiftsState, workspaceState, setViewLoginModal } =
    useLoginContext()
  const { deleteShiftMutation, updateShiftMutation } = useMutateShift()
  const [timeErrMsg, setTimeErrMsg] = useState('')

  const shift = shiftsState.filter((shift) => shift.id === eventSelected)[0]
  const title = workspaceState.filter(
    (Workspace) => Workspace.id === shift.workspace_id
  )[0]
  ...
  const submitShiftHandler = async (data: ShiftForm) => {
  ...
  return (
    ...
    >
      <form
        ...
        onSubmit={handleSubmit(submitShiftHandler)}
      >
        <div className="p-3">
          <div className="">
            <div className="flex flex-col justify-between items-center">
              ...
              <div>
                <span className="text-fuchsia-400 font-bold">U</span>pdate Shift
              </div>
              ...
              <div className="flex flex-col justify-center items-start">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    className="input-form cursor-pointer"
                    {...register('start_time')}
                    defaultValue={dayjs(shift.start_time).format(
                      'YYYY-MM-DDTHH:mm'
                    )}
                  />
                  <p className="text-rose-600">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    className="input-form cursor-pointer"
                    {...register('end_time')}
                    defaultValue={dayjs(shift.end_time).format(
                      'YYYY-MM-DDTHH:mm'
                    )}
                  />
                  <p className="text-rose-600">
